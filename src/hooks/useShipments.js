  ...
  const fetchShipments = useCallback(async (isBackgroundSync = false) => {
    try {
      ...
      const normalized = (data.data || []).map(s => ({
        ...
        expectedNextVessel: s.expected_next_vessel || s.expectedNextVessel || '',
        expectedVesselDate: s.expected_vessel_date || s.expectedVesselDate || '',
        ...
        claimStatus: s.claim_status || s.claimStatus || null,
        claimReference: s.claim_reference || s.claimReference || '',
        claimNotes: s.claim_notes || s.claimNotes || '',
        lateConfirmed: s.late_confirmed ?? s.lateConfirmed ?? false,
        lateConfirmedAt: s.late_confirmed_at || s.lateConfirmedAt || null,
        lateConfirmedBy: s.late_confirmed_by || s.lateConfirmedBy || '',
