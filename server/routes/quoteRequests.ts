...
router.post(
  ...
  [
    ...
    body('products.*.weight_kg').optional({ checkFalsy: true }).isFloat({ min: 0 }),
    body('products.*.value').optional({ checkFalsy: true }).isFloat({ min: 0 }),
    body('products.*.value_currency').optional({ nullable: true }).trim(),
